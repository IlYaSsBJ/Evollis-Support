...
- If I had three more days, my immediate priority would be moving the application from a mock frontend simulation to a production-ready full-stack architecture. First, I would move the Gemini API call server-side (using Next.js API routes or an Express server) so the API key is never exposed in the browser, adding proper rate limiting per session to prevent abuse. Second, I would replace the ephemeral React state with a real database (like Supabase or PostgreSQL) to persist chat histories; currently, if a user refreshes the page, the entire conversation is lost, and storing this data would allow human agents to pick up exactly where the AI left off. Third, I would implement an advanced Input Guardrail layer to pre-filter toxic content, spam, and malicious prompt-injection payloads before they ever reach the core AI pipeline. Fourth, I would introduce Semantic Caching using a vector database (like Redis or Upstash) to instantly serve cached answers for highly repetitive user inquiries, drastically slashing operational token costs. Fifth, I would implement a functional escalation pipeline: when the AI flags an escalation, the backend would use a service like SendGrid to instantly email the support team with the transcript and the escalation reason, or automatically generate a ticket in a CRM like Zendesk. Sixth, I would expand my test suite to rigorously audit the system prompt for vulnerabilities, edge-case classification over-triggering (such as separating general policy inquiries from explicit user actions), and potential adversarial prompt injections. Finally, I would connect the agent to a secure read-only database to verify 8-digit Contract IDs or real-time shipping statuses, allowing the agent to self-resolve Order and Billing inquiries rather than simply deferring them to human agents.
